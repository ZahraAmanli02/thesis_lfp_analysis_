# ============================================================
# 11C3_BOOTSTRAP_WEIGHT_CHANGE_PER_PHASE.PY
#
# Purpose:
#   Per-phase overview panel for RQ2 (weight-CHANGE version).
#   Four rows (phases A–D), two columns (HFD | CTRL). Same
#   visual language as 10b4 and 11c3 (absolute).
#
#   The HFD-vs-CTRL side-by-side layout is the substantive
#   test: HFD is where weight_delta actually moves (positive
#   trajectory over the diet), so we expect visible R²>0
#   cells; CTRL should sit around zero (no delta to predict).
#
# Input:
...
#
# Output:
#   outputs/11c3_bootstrap_weight_change_per_phase/
#       11c3_weight_change_per_phase.png
# ============================================================
...
OUT_DIR = os.path.join(OUTPUT_DIR, "11c3_bootstrap_weight_change_per_phase")
os.makedirs(OUT_DIR, exist_ok=True)
OUT_PNG = os.path.join(OUT_DIR, "11c3_weight_change_per_phase.png")
...
for row_i, phase in enumerate(ESTROUS_PHASES):
    for col_i, subset in enumerate(SUBSET_COLUMNS):
        ...
        if pd.isna(n_rec):
            title = (f"{subset}  |  Phase {phase} — "
                     f"{PHASE_NAMES[phase]}   (no data)")
        else:
...
fig.suptitle(
    "Per-phase overview — weight-CHANGE regression (RQ2)   ·   "
    "HFD vs CTRL   ·   target = body_weight - baseline_of_that_mouse   ·   "
