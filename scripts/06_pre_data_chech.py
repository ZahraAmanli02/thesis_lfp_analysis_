...
#   - chi-square test on estrous × group balance
...
#   - 06_estrous_chi_square_<CABLE>.csv  (chi-square results)
#
# Why this exists:
...
#       06_estrous_chi_square_<CABLE>.csv
# ============================================================
...
# ============================================================
# 5b. CHI-SQUARE TEST: ESTROUS × GROUP BALANCE
# ------------------------------------------------------------
# Tests whether estrous-phase distribution differs between HF and CTRL,
# computed on BOTH the full clean cohort and the diet-only subset
# (the latter is what 06a models will actually use). Results are also
# saved to CSV so Chapter 4 text can pull the numbers automatically.
# ============================================================

print("\n" + "=" * 60)
print("5b. CHI-SQUARE TEST: ESTROUS × GROUP")
print("=" * 60)

# Full clean cohort
ct_full = pd.crosstab(df["estrous_phase"], df["group"])
chi2_full, p_full, dof_full, _ = stats.chi2_contingency(ct_full)
print(f"\nFull clean cohort (n={len(df)}):")
print(f"  chi2({dof_full}) = {chi2_full:.3f}, p = {p_full:.4f}")

# Diet-only subset (06a model input)
ct_diet = pd.crosstab(diet_only["estrous_phase"], diet_only["group"])
chi2_diet, p_diet, dof_diet, _ = stats.chi2_contingency(ct_diet)
print(f"\nDiet-only subset (n={len(diet_only)}):")
print(f"  chi2({dof_diet}) = {chi2_diet:.3f}, p = {p_diet:.4f}")

# Save to CSV
chi_summary = pd.DataFrame({
    "cable":   [CABLE, CABLE],
    "scope":   ["full_clean", "diet_only"],
    "n":       [len(df), len(diet_only)],
    "chi2":    [chi2_full, chi2_diet],
    "dof":     [dof_full, dof_diet],
    "p_value": [p_full, p_diet],
})
chi_out_path = os.path.join(OUT_DIR, f"06_estrous_chi_square_{CABLE}.csv")
chi_summary.to_csv(chi_out_path, index=False)
print(f"\nSaved: {chi_out_path}")
...
ax.set_ylabel("Recordings")
p_str = f"p = {p_diet:.3f}" if p_diet >= 0.001 else "p < 0.001"
ax.set_title(f"C. Estrous balance (diet-only)\n"
             f"chi2({dof_diet}) = {chi2_diet:.2f}, {p_str}",
             fontweight="bold", loc="left")
ax.legend(frameon=False, fontsize=9)
