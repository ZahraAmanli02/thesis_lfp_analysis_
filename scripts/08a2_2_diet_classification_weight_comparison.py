# ============================================================
# 08A2_2_DIET_CLASSIFICATION_WEIGHT_COMPARISON.PY
# Purpose:
...
#
# Output:
#   outputs/08a2_2_diet_classification_weight_<CABLE>/
#       08a2_2_diet_classification_weight_results_<CABLE>.csv   (long)
#       08a2_2_diet_classification_weight_summary_<CABLE>.txt   (readable)
#       08a2_2_diet_classification_weight_<CABLE>.png           (figure)
# ============================================================
...
OUT_DIR = os.path.join(OUTPUT_DIR, f"08a2_2_diet_classification_weight_{CABLE}")
os.makedirs(OUT_DIR, exist_ok=True)
OUT_CSV = os.path.join(
    OUT_DIR, f"08a2_2_diet_classification_weight_results_{CABLE}.csv")
OUT_TXT = os.path.join(
    OUT_DIR, f"08a2_2_diet_classification_weight_summary_{CABLE}.txt")
OUT_PNG = os.path.join(
    OUT_DIR, f"08a2_2_diet_classification_weight_{CABLE}.png")
...
# The feature sets compared in this script
FEATURE_SETS = {
    "lfp_only":            LFP_FEATURES,
    "weight_only":         WEIGHT_FEATURE,
    "lfp_plus_weight":     LFP_FEATURES + WEIGHT_FEATURE,
    "weight_plus_estrous": WEIGHT_FEATURE + ESTROUS_DUMMIES,
    "lfp_weight_estrous":  LFP_FEATURES + WEIGHT_FEATURE + ESTROUS_DUMMIES,
}

C_SETS = {"lfp_only": "#4C72B0", "weight_only": "#DD8452",
          "lfp_plus_weight": "#1D9E75", "weight_plus_estrous": "#937860",
          "lfp_weight_estrous": "#C44E52"}
...
# Estrous dummies (A reference), all four categories enforced so the
...
for c in ESTROUS_DUMMIES:
    if c not in est_dummies.columns:
        est_dummies[c] = 0
df = pd.concat([df, est_dummies[ESTROUS_DUMMIES].astype(float)], axis=1)

all_needed = sorted(set(LFP_FEATURES + WEIGHT_FEATURE + ESTROUS_DUMMIES))
