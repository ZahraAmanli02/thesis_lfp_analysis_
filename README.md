...
The major analyses were performed separately for Cable 1 and Cable 3 to assess the reproducibility of the findings across independent recording configurations.

## Main Research Questions

### RQ1 — Diet Classification

Can dietary group be decoded from a single electrophysiological recording?
...
### RQ3 — Independence of Diet and Weight Information

Do the same electrophysiological features encode both dietary group and body weight?

The significant feature cells identified for diet classification and body-weight prediction were compared across frequency bands, band-power ratios, estrous phases, and cables.

### RQ4 — Cross-Cable Reproducibility

Are the main findings reproducible across Cable 1 and Cable 3?

The analysis pipeline was applied independently to both cables. Reproducibility was evaluated at the level of signal sparsity, estrous-phase dependence, frequency-band localization, and predictive performance.
...
Relative band power represents the power of a particular band relative to the total spectral power between 1 and 140 Hz.

### Band-Power Ratios

Pairwise ratios between frequency bands were calculated to assess the balance between oscillatory components.
...
## Reproducibility Across Cables

Cable 1 and Cable 3 were generally analysed independently.

The use of two independent cables allowed the project to assess whether the main findings were reproducible across different recording configurations.

Cross-cable reproducibility was evaluated based on:

- the number of significant feature cells;
- the frequency bands containing the strongest signals;
- the dependence of results on estrous phase;
- classification and regression performance;
- the separation between diet-related and weight-related features.

The exact identity of significant feature cells was not required to be identical across the two cables. Instead, reproducibility was interpreted at the level of the broader biological and analytical pattern.
...
```bash
python scripts/01_load_recordings.py
python scripts/01b_add_metadata.py
python scripts/01c_filter_recordings.py
```
